...
class Catalog extends XBaseModel
{
    ...
    static public function lite ($parentId, array $array = array(), $params = array())
    {
        ...
        foreach ((array)$eachArr as $row) {
            if ($row['parent_id'] == $parentId){
                ...
                if (isset($params['all'])) {
                    $tem = self::lite($row[id], $eachArr, $params);
                    $arr = is_array($tem) ? array_merge($arr, $tem) : $arr;
                }
            }
        } 
        return $arr;
     ...
     * @return array
     */
    static public function liteAll ($id, array $array = array(), $params = array())
    {
        $eachArr = empty($array)? XXcache::system('_catalog', 86400): $array;
        $return = self::recur_n($eachArr, $id);
        return $return;
    }
    ...
    * 反向递归         
    */
    static public function recur_n($arr, $id, $children= array()) {   
        $return = array();
        $s = self::item($id, $arr);
        if ($s['parent_id'] != 0) {
            $return = self::recur_n($arr, $s['parent_id']);
        }else{
            return self::recur_p($arr, $s['id']);
        }
        return $return;   
    }   
     ...
     * 正向递归          
     */       
    static public function recur_p($arr, $pid) {      
        $return = array();
        $i = 0;
        foreach ($arr as $v) {   
            if ($pid == $v['parent_id']) {
                $return[$i]['s'] = $v;
                if(!empty($c = self::recur_p($arr, $v['id']))){
                    $return[$i]['c'] = $c;
                }
     ...
     * @param array $array
     */
    static public function itemTop ($id, array $array = array())
    {
        if(empty($id))
            return ;
        $s = self::item($id, $array);
        if ($s['parent_id'] != 0) {
            return self::itemTop($s['parent_id'], $array);
        }else{
            return $s;
        }
    }
