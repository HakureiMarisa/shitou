...
class PostController extends XFrontBase
{
  ...
  public function actionIndex() {
    ...
    if ( $catalog && $catalogArr ) {
      if ( $catalogArr['display_type'] == 'list' ) {
        ...
        $ids = Catalog::lite(intval($catalogArr['id']), array(), array('all' => true, 'field' => 'id'));
        $ids[] = $catalogArr['id'];
        //print_r($ids);
        $resultArr = self::_catalogList( array( 'catalog'=>$ids, 'pageSize'=>$catalogArr['page_size']  ));
      }else {
      ...
      $resultArr['catalogTopArr'] = Catalog::itemTop( $catalogArr['id'], $this->_catalog );
    ...
    $tplVars = array(
      ...
      'catalogChild'=>Catalog::liteAll(intval($resultArr['catalogTopArr']['id'])),
    );
  ...
  $tplVar = array(
    ...
    'attrVal'=>$attrVal,
    'catalogArr'=>$catalogArr,
    'catalogChild'=>Catalog::liteAll(intval( $bagecmsShow->catalog_id)),
    'catalogTopArr'=>Catalog::itemTop( $bagecmsShow->catalog_id, $this->_catalog )
    );
